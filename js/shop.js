    ...
    for (let i = 0; i < playButtons.length; i++) {
      $(playButtons[i]).on("click", function() {
        if ($(this).attr("data-iframe-ref") === "video_one") {
          ...
          $("#video_player").css(
            "background-image",
            "url(../wp-content/themes/excelpro/img/placeholder_laptop.png)"
          );
          $("#video-1")[0].play();
          $("#video-2")[0].pause();
          $("#video-3")[0].pause();
        ...
        if ($(this).attr("data-iframe-ref") === "video_three") {
          ...
          $("#video_player").css(
            "background-image",
            "url(../wp-content/themes/excelpro/img/placeholder_phone.png)"
          );
          $("#video-2")[0].pause();
          $("#video-1")[0].pause();
          $("#video-3")[0].play();
    ...
    $(window).load(function() {
      if (
        window.location.href.indexOf(
          `${window.location.origin}/shop/?add-to-cart`
        ) === 0
      ) {
